...
from typedef_cl import ComponentList                                        #структура списка компонентов
...
def export(data, address, **kwargs):
    ...
    book_style = {
        ...
        'col' : {
            ...
            'parametric'            : {'width':  4.0, 'valign': 'vcenter'},
        ...
        'comment' : {
            'header'                : {
                'parametric'            : {'x_scale': 2.0, 'y_scale': 0.75}
            }
        }
    }
    ...
    if len(data) > 0:
        with xlsxwriter.Workbook(address, {'in_memory': True, 'strings_to_numbers': False}) as workbook:
            ...
            wb_format_col_partnumber  = None if book_style['col']['partnumber']  is None else workbook.add_format(book_style['col']['partnumber'])
            wb_format_col_parametric  = None if book_style['col']['parametric']  is None else workbook.add_format(book_style['col']['parametric'])
            ...
            wb_format_col_subst_orig_partnumber = None if book_style['col']['subst_orig_partnumber'] is None else workbook.add_format(book_style['col']['subst_orig_partnumber'])
            ...
            wb_format_col_subst_partnumber      = None if book_style['col']['subst_partnumber']      is None else workbook.add_format(book_style['col']['subst_partnumber'])
            ...
            format_comment_header_parametric = book_style.get('comment', {}).get('header', {}).get('parametric', {})
            ...
            for cl in data:
                ...
                if cl.components is None and cl.accessories is not None:
                    ...
                    if content_accs_location == 'sheet':
                        ...
                        for i in range(content_accs_indent): cl_combined.entries.append(ComponentList.ComponentEntry())     #приводит к 0 в столбце количества
                ...
                for sublist in sublists:
                    ...
                    worksheet.write_row(0, 0, [locale.translate(lcl.export_cl_xlsx.HEADER_DESIGNATOR), locale.translate(lcl.export_cl_xlsx.HEADER_COMPONENT_TYPE), locale.translate(lcl.export_cl_xlsx.HEADER_PARTNUMBER), locale.translate(lcl.export_cl_xlsx.HEADER_PARAMETRIC), locale.translate(lcl.export_cl_xlsx.HEADER_DESCRIPTION), locale.translate(lcl.export_cl_xlsx.HEADER_PACKAGE), locale.translate(lcl.export_cl_xlsx.HEADER_MANUFACTURER), locale.translate(lcl.export_cl_xlsx.HEADER_QUANTITY), locale.translate(lcl.export_cl_xlsx.HEADER_NOTE)], wb_format_row_header)
                    header_parametric_comment = locale.translate(lcl.export_cl_xlsx.HEADER_PARAMETRIC_COMMENT)
                    if header_parametric_comment: worksheet.write_comment('D1', header_parametric_comment, format_comment_header_parametric)
                    row_index = 1    #начальная строка для записи данных
                    for entry in sublist.entries:
                        if entry.flag == ComponentList.FlagType.OK:
                            row_format = wb_format_row_ok
                            row_height = row_heights.get('ok', None)
                        elif entry.flag == ComponentList.FlagType.WARNING:
                            row_format = wb_format_row_warning
                            row_height = row_heights.get('warning', None)
                        elif entry.flag == ComponentList.FlagType.ERROR:
                        ...
                        partnumber   = replace_nones(entry.partnumber,   '', False)
                        if entry.parametric: parametric = locale.translate(lcl.export_cl_xlsx.VALUE_PARAMETRIC_TRUE)
                        else:                parametric = locale.translate(lcl.export_cl_xlsx.VALUE_PARAMETRIC_FALSE)
                        ...
                        worksheet.write_row(row_index, 0, [
                            ...
                            format_singlevalue_delimiter.join(partnumber),
                            parametric,
                    ...
                    worksheet.set_column(2, 2, col_widths['partnumber'],  wb_format_col_partnumber)   #Артикул
                    worksheet.set_column(3, 3, col_widths['parametric'],  wb_format_col_parametric)   #Параметрический артикул
                    worksheet.set_column(4, 4, col_widths['description'], wb_format_col_description)  #Описание
                    worksheet.set_column(5, 5, col_widths['package'],     wb_format_col_package)      #Корпус
                    worksheet.set_column(6, 6, col_widths['mfr'],         wb_format_col_mfr)          #Производитель
                    worksheet.set_column(7, 7, col_widths['quantity'],    wb_format_col_quantity)     #Кол-во
                    worksheet.set_column(8, 8, col_widths['note'],        wb_format_col_note)         #Примечание
                ...
                if cl.substitutes is not None:
                    ...
                    #запись данных списка компонентов
                    worksheet.write_row(0, 0, [locale.translate(lcl.export_cl_xlsx.HEADER_ORIGINAL_PARTNUMBER), locale.translate(lcl.export_cl_xlsx.HEADER_ORIGINAL_MANUFACTURER), locale.translate(lcl.export_cl_xlsx.HEADER_ORIGINAL_QUANTITY), locale.translate(lcl.export_cl_xlsx.HEADER_DESIGNATOR), locale.translate(lcl.export_cl_xlsx.HEADER_SUBSTITUTE_QUANTITY), locale.translate(lcl.export_cl_xlsx.HEADER_SUBSTITUTE_PARTNUMBER), locale.translate(lcl.export_cl_xlsx.HEADER_SUBSTITUTE_MANUFACTURER), locale.translate(lcl.export_cl_xlsx.HEADER_SUBSTITUTE_NOTE)], wb_format_row_header)
                    ...
                    for entry in cl.substitutes.entries:
                        ...
                        for substitute_group in entry.substitute_group:
                            ...
                            for substitute in substitute_group.substitute:
                                partnumber   = replace_nones(substitute.partnumber,   '', False)
                                manufacturer = replace_nones(substitute.manufacturer, '', False)
                                note         = replace_nones(substitute.note,         '', False)
                                row_index += 1
                                group_height += 1
                                worksheet.write_row(row_index, 5, [partnumber, manufacturer, note], row_format)
                                if row_height is not None: worksheet.set_row(row_index, row_height)
                        ...
                        primary_partnumber   = replace_nones(entry.primary_partnumber,   '', False)
                        ...
                        if entry_height > 1:
                            worksheet.merge_range(row_index - (entry_height - 1), 0, row_index, 0, primary_partnumber, row_format)
                            ...
                            worksheet.write_row(row_index, 0, [primary_partnumber, primary_manufacturer, primary_quantity], row_format)

                    worksheet.set_column(0, 0, col_widths['subst_orig_partnumber'], wb_format_col_subst_orig_partnumber)
                    ...
                    worksheet.set_column(5, 5, col_widths['subst_partnumber'],      wb_format_col_subst_partnumber)
