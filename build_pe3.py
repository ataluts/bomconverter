...
def build(data, **kwargs):
    ...
    setting_format_designator_delimiter             = kwargs.get('format_designator_delimiter', [', ', '\u2013'])
    ...
    if len(components.entries) > 0:
        ...
        for component in components.entries:
            ...
            value = PE3.Element.Value(value.label, value.annotation)
            if component.GNRC_designator is None:
                ...
                designator = Designator('', None, '', 0, component.GNRC_accessory_parent.GNRC_designator)   #делаем специальный десигнатор для таких элементов
                ...
                for element in pe3.elements:
                    if element.designator.name == designator.name:
                        if element.dominant.value.match(value):
                            element.add_channel(component.GNRC_accessory_parent.GNRC_designator, value, quantity)
                            break
                else:
                    pe3.elements.append(PE3.Element(designator, value, quantity))
            else:
                #есть десигнатор -> консолидируем каналы в общий базовый элемент перечня
                for element in pe3.elements:
                    if element.designator.name == component.GNRC_designator.name:
                        element.add_channel(component.GNRC_designator.channel, value , quantity)
                        break
                else:
                    pe3.elements.append(PE3.Element(component.GNRC_designator, value, quantity))
                ...
                if element.designator.prefix is None:
                    if len(element.dominant.channels) > 0:
                        ...
                        for parent in element.dominant.channels:
                            ...
                            parents += assemble.assemble_designator(parent, **kwargs) + setting_format_designator_delimiter[0]
                        parents = parents[:-len(setting_format_designator_delimiter[0])] + setting_format_accs_parent_enclosure[1]
        ...
        for element in pe3.elements:
            ...
            if element.dominant is not None:
                value = PE3.Entry.Value(element.designator.name, element.dominant.value.label, element.dominant.quantity, element.dominant.value.annotation)
            if len(pe3.entries) > 0:
                ...
                if value is not None and value.match(pe3.entries[-1].baseline):
                    ...
                    if pe3.entries[-1].prefix == element.designator.prefix and pe3.entries[-1].local_numbers[-1] == element.designator.number - 1:
                        ...
                        pe3.entries[-1].add_value(element.designator.number, value, setting_format_designator_delimiter)
                        pe3.entries[-1].add_deviations(_build_deviations(element, setting_format_designator_delimiter))
                        continue
            #либо список пуст либо текущая запись отличается от последней в списке
            pe3.entries.append(PE3.Entry(element.designator.prefix, element.designator.number, value))
            pe3.entries[-1].add_deviations(_build_deviations(element, setting_format_designator_delimiter))
        ...
        for entry in pe3.entries:
            ...
            if isinstance(group_name[0], (tuple, list)):
                group_name = (group_name[0][locale.value], group_name[1][locale.value])     #если словарь с локализацией то выбираем нужную
        ...
        for group in pe3.groups:
            ...
            for index, entry in enumerate(group.entries):
                ...
                if entry.baseline is not None:
                    pe3.rows.append(PE3.Row(entry.baseline.designator,
                ...
                for jndex, deviation in enumerate(entry.deviations):
                    ...
                    if jndex > 0 or entry.baseline is not None:
                        for i in range(setting_format_table_entry_deviation_indent):
                            pe3.rows.append(PE3.Row())
                    pe3.rows.append(PE3.Row(deviation.value.designator,
...
def _build_deviations(element, format_desig_delimiter):
    ...
    for variant in element.variants:
        for channel, quantity in zip(variant.channels, variant.quantities):
            #получаем значение
            value = PE3.Entry.Value(element.designator.name + channel.full, variant.value.label, quantity, variant.value.annotation)
            if len(deviations) > 0:
                #список не пуст
                if value.match(deviations[-1].value):
                    ...
                    if deviations[-1].channel_numbers[-1] == channel.number - 1:
                        ...
                        continue
            #либо список пуст либо текущая запись отличается от последней в списке
            deviations.append(PE3.Entry.Deviation(element.designator.number, channel.number, value))
    return deviations